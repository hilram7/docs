...
Sensitive Data Discovery can identify any file type, extract text, and extract metadata from the
following formats. It will also identify file types and extract metadata of any attachments. If the
attachment’s file type is a scan-able format, then it can extract text from the attachment as well.
...
Sensitive Data Discovery can identify file type and extract only metadata from images and
