---
title: "Web Application & Web Application URL Levels Reports"
description: "Web Application & Web Application URL Levels Reports"
sidebar_position: 60
---

# Web Application & Web Application URL Levels Reports

The following reports are available at the web application and web application URL levels and
provide information for SharePoint on-premise farms:

- [Effective Policy Report](/docs/accessinformationcenter/11.6/resourceaudit/sharepoint/webapplication/effectivepolicy.md)
- [Policy Report](/docs/accessinformationcenter/11.6/resourceaudit/sharepoint/webapplication/policy.md)

