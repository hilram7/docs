---
title: "Administrator Overview"
description: "Administrator Overview"
sidebar_position: 30
---

# Administrator Overview

Access Information Center administrators have access to the Configuration interface where there
application settings reside. This topic covers:

- [Getting Started](/docs/accessinformationcenter/12.0/admin/gettingstarted.md)
- [First Launch](/docs/accessinformationcenter/12.0/admin/firstlaunch.md)
- [Navigation](/docs/accessinformationcenter/12.0/admin/navigate/navigate.md)
- [Configuration Interface Overview](/docs/accessinformationcenter/12.0/admin/configuration/overview.md)
- [Additional Configuration Options](/docs/accessinformationcenter/12.0/admin/additionalconfig/overview.md)
- Instructions on how different users access the application (see the [URL & Login](/docs/accessinformationcenter/12.0/admin/login/login.md) topic)
- [Troubleshooting](/docs/accessinformationcenter/12.0/admin/troubleshooting/overview.md)

