---
title: "Team Member, Resource & Subfolder Level Reports"
description: "Team Member, Resource & Subfolder Level Reports"
sidebar_position: 40
---

# Team Member, Resource & Subfolder Level Reports

The following reports are available at the Team Member, Resource, and Subfolder level:

- Activity Report – Displayed but not populated at the Team Member, Resource, and Subfolder level
- [Permissions Report](/docs/accessinformationcenter/12.0/resourceaudit/dropbox/teammemberresource/permissions.md)
- [Sensitive Content Report](/docs/accessinformationcenter/12.0/resourceaudit/dropbox/teammemberresource/sensitivecontent.md)

