---
title: "AD Monitoring Configuration Window"
description: "AD Monitoring Configuration Window"
sidebar_position: 10
---

# AD Monitoring Configuration Window

On the Monitored Domains tab, select the domain and click **Edit** to open the AD Monitoring
Configuration window.

![AD Monitoring Configuration - Global Filters Tab](/images/activitymonitor/8.0/admin/monitoreddomains/admonitoringconfiguration/globalfilterstab.webp)

This initially configured when the AD Agent is deployed to a domain controller. However, the
monitoring configuration can be edited after that. Use the following tabs to modify monitoring of AD
events:

- [Global Filters Tab](/docs/activitymonitor/8.0/admin/monitoreddomains/admonitoringconfiguration/globalfilters.md)
- [Changes Tab](/docs/activitymonitor/8.0/admin/monitoreddomains/admonitoringconfiguration/changes.md)
- [Authentication Tab](/docs/activitymonitor/8.0/admin/monitoreddomains/admonitoringconfiguration/authentication.md)
- [Replication Tab](/docs/activitymonitor/8.0/admin/monitoreddomains/admonitoringconfiguration/replication.md)
- [LSASS Guardian Tab](/docs/activitymonitor/8.0/admin/monitoreddomains/admonitoringconfiguration/lsassguardian.md)
- [LDAP Monitor Tab](/docs/activitymonitor/8.0/admin/monitoreddomains/admonitoringconfiguration/ldapmonitor/ldapmonitor.md)

