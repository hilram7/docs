...
See the [File Archive Repository Option](/docs/activitymonitor/8.0/requirements/adagent/activity/filearchive.md) topic for additional information on that
option.
...
Follow the steps to setup integration between Activity Monitor and Access Analyzer through an API
server.
...
- The default **API server port (TCP)** is 4494, but it can be modified if desired. Ensure the
  modified port is also used by Access Analyzer.
...
- Configure the following:
    ...
    - By default, the **Secret Expires** in 3 days. That means it must be used in the Access
      Analyzer Connection Profile within 72 hours or a new secret will need to be generated. Modify
      if desired.
...
**Step 1 –** On the **Settings** > **Connection** node of the Access Analyzer Console, select the
...
The Access Analyzer requires additional configurations in order to collect domain activity data.
