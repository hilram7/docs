...
In Usercube, position changes are made through workflows or through synchronization to the HR
system.
...
Just like onboarding, the entitlement fulfillment can be performed either by using Usercube's
suggestions for the needed entitlements and adjusting them, or trusting Usercube with an automated
fulfillment.

Usercube's calculations for entitlement assignments rely on heuristics, through identities' key
properties called
[dimensions](/docs/identitymanager/6.1/introduction-guide/overview/entitlement-management/index.md).
...
![Records Origin - Three-Entity Model](/images/identitymanager/6.1/integration-guide/identity-management/joiners-movers-leavers/position-change/recordsorigin_firstmodel.webp)
...
[provisioning](/docs/identitymanager/6.1/integration-guide/provisioning/index.md) of
...
to make sure that all rules are applied based on the right input? These issues imply complex C#
expressions in provisioning rules.
...
Usercube, where all values in one datasheet are valid on a given time period.
...
>
> ![User Example](/images/identitymanager/6.1/integration-guide/identity-management/joiners-movers-leavers/position-change/recordsorigin_userexample.webp)
>
> Over time, the three entities are as follows:
>
> ![Example - Timelines](/images/identitymanager/6.1/integration-guide/identity-management/joiners-movers-leavers/position-change/recordsorigin_timelines.webp)
>
...
>
> ![Example - Contexts](/images/identitymanager/6.1/integration-guide/identity-management/joiners-movers-leavers/position-change/recordsorigin_contexts.webp)
...
The complexity that comes from the combination of all start and end dates is tackled by Usercube's
engine when it generates users' contexts. As the start and end dates of each value are pre-computed
by Usercube, this user model highly simplifies provisioning rules.
...
![Records Origin - Final Model](/images/identitymanager/6.1/integration-guide/identity-management/joiners-movers-leavers/position-change/recordsorigin_thirdmodel.webp)
...
Plus, Usercube does not need to archive old data, because records and contexts are used only to
simplify the application of provisioning rules. As only valid values are provisioned, there is no
need to keep track.
