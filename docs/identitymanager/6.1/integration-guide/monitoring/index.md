...
[See the list of existing logs](/docs/identitymanager/6.1/integration-guide/monitoring/references/index.md).
...
Every log message is associated with a log level and a user-defined _namespace_. Usercube provides
the _Usercube_ namespace, associated with logs relevant to the user.
...
- `Override` allows the user to set a different minimum log level for logs from a specific namespace
  (see [Custom namespaces](#custom-namespaces)).

    Within Usercube, the following example is a good practice: default logs with a priority lower
    than `Error` are filtered out, except for log messages from the _Usercube_ namespace.
...
using the previous `outputTemplate` format.
...
destination such as standard consoles, files and logging services.
...
[dedicated How To page](/docs/identitymanager/6.1/integration-guide/monitoring/how-tos/qradar-setting/index.md)
.
...
Usercube offers the ability to download the application logs directly through the User Interface
(UI) via the **Monitoring** screen in the **Administration** section on the Dashboard.
...
of the appsettings called `LogsPath` indicating the path where the application logs are located:
...
separate files within a directory, provide the path to the directory and Usercube will handle
providing the most recent logs.
