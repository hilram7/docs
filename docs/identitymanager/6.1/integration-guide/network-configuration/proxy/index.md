...
Usercube server or agent can be configured to go through a proxy server to access internal or
external web resources.

## Introduction

A Usercube agent often needs to access internal or external systems using the HTTP protocol. It may
easily be configured to use a proxy server through which all or part of the HTTP traffic will be
routed.
...
- At OS level
- At user level: for the user running the Usercube server or agent
...
running the Usercube agent all tasks started by the agent will inherit the proxy environment
variables.
...
To test that a Usercube agent using a proxy server can reach the Usercube server: Go to the
`<USERCUBE_HOME>/Runtime` directory.
...
to the Usercube server or agent.
