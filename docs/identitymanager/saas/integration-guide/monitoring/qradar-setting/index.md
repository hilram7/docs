...
This guide shows how to use the logging configuration (Serilog) to send Identity Manager's logs into
a log management system, potentially using specific plug-ins to parse the logs.
...
Identity Manager's Device Support Module is a plug-in that allows your QRadar system to parse
Identity Manager's logs, when producing a JSON output.
...
means that, once Serilog is configured to send logs to QRadar, performing a few actions in Identity
...
3. Add a **MinimumLevel** section to define which logs are to be sent to the log management system.
    ...
    In order to be sent to any system, Identity Manager's logs must be configured with
