...
![Subscription window](/images/threatprevention/7.4/reportingmodule/investigations/options/subscription.webp)
...
is: Subscription to `{{Investigation_Name}}` at `{{Date}}`. These variables will be replaced with
the name of the investigation and the date of the report. The + button opens a variable menu for the
field.
